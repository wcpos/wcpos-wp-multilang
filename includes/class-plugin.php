...
class Plugin {
	public const STORE_LANGUAGE_META_KEY = '_wcpos_wp_multilang_language';
	...
	public function load_textdomain(): void {
		load_plugin_textdomain( 'wcpos-wp-multilang', false, dirname( plugin_basename( dirname( __DIR__ ) . '/wcpos-wp-multilang.php' ) ) . '/languages' );
	}
	...
	public function filter_product_query( array $args, WP_REST_Request $request ): array {
		if ( ! $this->is_wp_multilang_supported() ) {
			return $args;
		}
	...
	public function enqueue_store_edit_assets( string $hook_suffix ): void {
		...
		$languages = $this->get_wp_multilang_languages_for_js();
		if ( empty( $languages ) ) {
			return;
		}

		$default_language = $this->get_default_language();
		$default_label    = ! empty( $default_language ) ? $default_language : __( 'site default', 'wcpos-wp-multilang' );

		wp_enqueue_script(
			'wcpos-wp-multilang-store-edit',
			plugins_url( 'assets/js/store-language-section.js', dirname( __DIR__ ) . '/wcpos-wp-multilang.php' ),
		...
		wp_add_inline_script(
			'wcpos-wp-multilang-store-edit',
			'window.wcposWPMultilangStoreEdit = ' . wp_json_encode(
				array(
					'defaultLanguage' => $default_language,
					'languages'       => $languages,
					'strings'         => array(
						'sectionLabel'  => __( 'Language', 'wcpos-wp-multilang' ),
						'title'         => __( 'Store language', 'wcpos-wp-multilang' ),
						'description'   => __( 'Choose which WP Multilang language this store should use in WCPOS.', 'wcpos-wp-multilang' ),
						'help'          => __( 'Products in this store are filtered to the selected language. Leave this as default to use your site default language.', 'wcpos-wp-multilang' ),
						'defaultOption' => sprintf(
							/* translators: %s: language slug. */
							__( 'Default language (%s)', 'wcpos-wp-multilang' ),
							$default_label
						),
						'noLanguages'   => __( 'No WP Multilang languages found.', 'wcpos-wp-multilang' ),
					),
				)
			) . ';',
			'before'
		...
		if ( $store_id > 0 && class_exists( WCPOS_Pro_Stores::class ) ) {
			...
			if ( $authorized ) {
				...
				if ( ! empty( $store_language ) ) {
					return (string) apply_filters( 'wcpos_wp_multilang_resolved_language', $store_language, $request );
				}
			}
		}

		return (string) apply_filters( 'wcpos_wp_multilang_resolved_language', $default_language, $request );
	}
	...
	/**
	 * Resolve WP Multilang default language.
	 *
	 * @return string
	 */
	private function get_default_language(): string {
		...
		if ( function_exists( 'wpm_get_default_language' ) ) {
			$default_language = (string) wpm_get_default_language();
		}

		return (string) apply_filters( 'wcpos_wp_multilang_default_language', $default_language );
	}

	/**
	 * Check whether WP Multilang is active and supported.
	 *
	 * @return bool
	 */
	private function is_wp_multilang_supported(): bool {
		$supported = function_exists( 'wpm_get_default_language' ) && function_exists( 'wpm_get_languages' );
		if ( ! $supported ) {
			return (bool) apply_filters( 'wcpos_wp_multilang_is_supported', false );
		}

		$minimum_version = $this->get_minimum_wp_multilang_version();
		if ( '' !== $minimum_version ) {
			$current_version = defined( 'WPM_VERSION' ) ? (string) WPM_VERSION : '';
			if ( '' === $current_version || version_compare( $current_version, $minimum_version, '<' ) ) {
				$supported = false;
			}
		}

		return (bool) apply_filters( 'wcpos_wp_multilang_is_supported', $supported );
	}

	/**
	 * Optional minimum required WP Multilang version.
	 *
	 * Defaults to empty (no strict version gate) and can be set via filter.
	 *
	 * @return string
	 */
	private function get_minimum_wp_multilang_version(): string {
		return (string) apply_filters( 'wcpos_wp_multilang_minimum_version', '' );
	}
	...
	private function apply_language_constraints( array $args, string $language ): array {
		...
		$args['meta_query']  = $this->merge_language_meta_query( $existing_meta_query, $language );

		return $args;
	}

	/**
	 * Merge language filter into existing meta query.
	 *
	 ...
	 * @return array
	 */
	private function merge_language_meta_query( $existing, string $language ): array {
		$language_clause = $this->build_language_meta_query_clause( $language );

		if ( ! is_array( $existing ) || empty( $existing ) ) {
			return array( $language_clause );
		}

		if ( isset( $existing['key'] ) && isset( $existing['compare'] ) ) {
			return array(
				$existing,
				$language_clause,
			);
		...
		$existing[] = $language_clause;
		return $existing;
	}

	/**
	 * Build language clause that mirrors WP Multilang's post language filter.
	 *
	 * @param string $language
	 *
	 * @return array
	 */
	private function build_language_meta_query_clause( string $language ): array {
		return array(
			'relation' => 'OR',
			array(
				'key'     => '_languages',
			...
			array(
				'key'     => '_languages',
				// phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions.serialize_serialize -- WP Multilang stores _languages as serialized data.
	 ...
	 */
	private function get_wp_multilang_languages_for_js(): array {
		...
		foreach ( $languages as $slug => $language_data ) {
			if ( is_int( $slug ) && is_string( $language_data ) ) {
				$slug          = $language_data;
				$language_data = array();
			}
			...
			if ( is_array( $language_data ) ) {
				if ( isset( $language_data['name'] ) && is_string( $language_data['name'] ) && '' !== $language_data['name'] ) {
					$label = $language_data['name'];
				} elseif ( isset( $language_data['label'] ) && is_string( $language_data['label'] ) && '' !== $language_data['label'] ) {
